...
namespace Tenrai.Tests.SeasonTests
{
	...
	public class GetSeasonAsyncTests
	{
		...
		public GetSeasonAsyncTests(TenraiFixture tenraiFixture)
		{
		...
		{
			// When
			var func = _tenrai.Awaiting(x => x.GetSeasonAsync(year, Season.Fall));
		...
		{
			// When
			var func = this._tenrai.Awaiting(x => x.GetSeasonAsync(2021, Season));
		...
		{
			// When
			var season = await _tenrai.GetSeasonAsync(year, Season.Winter);
		...
		{
			// When
			var winter2000 = await _tenrai.GetSeasonAsync(2000, Season.Winter);
		...
		{
			// When
			var spring1970 = await _tenrai.GetSeasonAsync(1970, Season.Spring);
		...
		{
			// When
			var winter2017 = await _tenrai.GetSeasonAsync(2017, Season.Winter);
		...
		{
			// When
			var func = this._tenrai.Awaiting(x => x.GetSeasonAsync(1970, Season.Winter, page));
		...
		public async Task GetSeasonAsync_Winter2017SecondPage_ShouldParseYowamushiPedal()
		{
			// When
			const int page = 2;
			var winter2017 = await _tenrai.GetSeasonAsync(2017, Season.Winter, page);
