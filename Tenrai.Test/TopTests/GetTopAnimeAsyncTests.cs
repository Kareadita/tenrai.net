...
namespace Tenrai.Tests.TopTests
{
	...
	public class GetTopAnimeAsyncTests
	{
		...
		public GetTopAnimeAsyncTests(TenraiFixture tenraiFixture)
		{
		...
		{
			// When
			var top = await _tenrai.GetTopAnimeAsync();
		...
		{
			// When
			var func = _tenrai.Awaiting(x => x.GetTopAnimeAsync(page));
		...
		{
			// When
			var top = await _tenrai.GetTopAnimeAsync(2);
		...
		{
			// When
			var func = _tenrai.Awaiting(x => x.GetTopAnimeAsync(TopAnimeFilter.Airing, page));
		...
		{
			// When
			var top = await _tenrai.GetTopAnimeAsync(TopAnimeFilter.Airing) ;
		...
		{
			// When
			var top = await _tenrai.GetTopAnimeAsync(TopAnimeFilter.Airing, 2) ;
		...
		{
			// When
			var func = _tenrai.Awaiting(x => x.GetTopAnimeAsync(null));
		...
		public async Task GetTopAnimeAsync_DefaultSearchConfig_ShouldParseFrieren()
		{
			...
			// When
			var result = await _tenrai.GetTopAnimeAsync(searchConfig);
